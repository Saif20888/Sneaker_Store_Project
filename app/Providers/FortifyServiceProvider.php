...
class FortifyServiceProvider extends ServiceProvider
{
    ...
    /**
     * Throttle password-reset requests. Fortify hardcodes only `guest` middleware on
     * these routes with no config-driven limiter hook (unlike login), so without this
     * they're wide open to email-enumeration / mail-bombing.
     */
    private function throttlePasswordResetRoutes(): void
    {
        foreach (['password.email', 'password.update'] as $name) {
            Route::getRoutes()->getByName($name)?->middleware('throttle:5,1');
        }
    }
